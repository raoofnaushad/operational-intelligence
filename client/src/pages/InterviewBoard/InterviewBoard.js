...
const InterviewBoard = () => {
  ...
  const Modal = ({ isOpen, setIsModalOpen, onDetailsSubmit, interview }) => {
    ...
    // Add a new state for storing company options
    const [companyOptions, setCompanyOptions] = useState([]);

    // Fetch interview details and auto-populate form, including company options
    useEffect(() => {
      if (isOpen && interview) {
        fetch(`${apiUrl}/interview/get_details`, {
          method: "POST",
          credentials: "include",
          mode: "cors",
          headers: {
          ...
          .then((data) => {
            setFormData({
              ...
              interviewObjective: data.interviewObjective || "",
              aboutClient: data.aboutClient || "",
            });

            // Assuming the list of companies is returned under a key like "companyOptions"
            // Update this according to the actual key used in your response
    ...
    const handleSubmit = async (event) => {
      ...
      const data = {
        name: formData.name,
        title: formData.title || interview.title,
        company_name: formData.company,
        company_website: formData.companywebsite || interview.companywebsite,
        job_description: formData.jobDescription || interview.jobDescription,
        company_description: formData.aboutClient || interview.aboutClient,
        interview_description: formData.interviewObjective,
        date: new Date().toISOString().split("T")[0],
        start_time: new Date().toTimeString().split(" ")[0],
        finish_time: "", // Assuming this is handled elsewhere or not needed initially
        finished: false, // Assuming initial state
        meeting_summary: "", // Assuming this is to be filled out later or not needed initially
      };
    ...
    <>
      ...
      <>
        ...
        <Modal
          ...
          interview={location.state.interview}
        />
      </>
    </>
  );
