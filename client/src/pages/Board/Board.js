...
const Board = () => {
  ...
  const handleInterviewCardClick = (interview) => {
    // Check if the interview link is valid
    if (interview.link && interview.link !== "N/A") {
      // Open the interview link in a new tab
      window.open(interview.link, "_blank");

      // Navigate to /InterviewBoard with the interview object as state
      navigate("/InterviewBoard", { state: { interview } });
    } else {
      // Display a popup warning if the link is not valid
      alert("No Virtual Meeting link found.");
    }
  };
