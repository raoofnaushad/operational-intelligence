...
def determine_meeting_objective(summary):
    
    # Craft a system prompt that instructs the model on the task
    system_prompt = "Given the meeting summary, determine the most accurate meeting objective from the following options only: " + ", ".join(C.MEETING_OBJECTIVES) + "." + " The response should be the one among the options."
    ...
    # The model_response is expected to contain the chosen meeting objective
    print("Model's chosen meeting objective:", model_response)
...
def generate_key_notes(request):
    ...
    general_prompt = U.generate_prompt(["farpoint_general", "client_general", "client_details", "meeting_general"], 
                                        interview_details)
...
def get_embedding(text, model=C.EMBED_MODEL):
   text = text.replace("\n", " ")
   return client.embeddings.create(input = [text], model=model).data[0].embedding


def get_similarity(embedding1, embedding2, model="text-embedding-3-small"):
