...
def generate_prompt(prompt_keys, interview_details):
    ...
    # Check and modify interview_details as per instructions
    if not interview_details.get('clientwebsite'):
        interview_details['clientwebsite'] = '<not available>'
    
    # Remove 'client_details' from prompt_keys if company_description is missing or empty
    if 'client_details' in prompt_keys and not interview_details.get('company_description', '').strip():
        prompt_keys = [key for key in prompt_keys if key != 'client_details']
