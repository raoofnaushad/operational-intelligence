...
def update_interview_question_status_logic(meeting_id, question_id, valid_value):
    conn = None
    cur = None
    try:
        conn = get_db_connection()
        ...
        # SQL query to update the 'valid' column to the provided value
        cur.execute("""
            UPDATE interviewQuestions
            SET valid = %s
            WHERE interview_id = %s AND id = %s
        """, (valid_value, meeting_id, question_id))
        ...
        return "Interview question status updated successfully."
    except Exception as e:
        logger.error(f"An error occurred while updating the interview question status: {e}", exc_info=True)
        return f"Error: {str(e)}"
    finally:
        if cur is not None:
            cur.close()
...
def store_user_access_token(user_info, access_token, refresh_token):
    conn = None
    cur = None
    try:
        # Connect to your postgres DB
        ...
        if user_id:
            # Update the existing user's access token, refresh token, and last_loggedin
            cur.execute("UPDATE oiusers SET access_token = %s, refresh_token = %s, last_loggedin = %s, name = %s WHERE emailid = %s;", 
                        (access_token, refresh_token, now, user_name, user_email))
        else:
            # Insert a new user with the given email, name, access token, refresh token, and created_time
            # Assuming created_time and last_loggedin are the same at the point of user creation
            cur.execute("INSERT INTO oiusers (name, emailid, role, team, access_token, refresh_token, created_time, last_loggedin) VALUES (%s, %s, NULL, NULL, %s, %s, %s, %s);", 
                        (user_name, user_email, access_token, refresh_token, now, now))
...
def retrieve_user_access_token(user_email):
    ...
    try:
        ...
        # Retrieve access token and refresh token for the given email
        cur.execute("SELECT access_token, refresh_token FROM oiusers WHERE emailid = %s;", (user_email,))
        result = cur.fetchone()

        if result:
            # Construct a dictionary with both tokens
            tokens = {
...
def get_user_id_and_name_by_email(email_id):
    """
    Fetches the user ID and name from the oiusers table based on the provided email ID.
    
    Args:
        email_id (str): The email ID of the user.
        
    Returns:
        tuple: A tuple containing the user ID and name if found, (None, None) otherwise.
    """
    conn = None
    try:
        conn = get_db_connection()
        ...
        # Execute query to find user ID and name by email ID
        cur.execute("SELECT id, name FROM oiusers WHERE emailid = %s", (email_id,))
        result = cur.fetchone()

        # Return user ID and name if found
        if result:
            return result[0], result[1]  # user_id, user_name
        else:
            return None, None
    except Exception as e:
        print(f"An error occurred while fetching user ID and name: {e}")
        return None, None
    finally:
        if conn:
            conn.close()
...
def save_chat_interaction(user_id, text, generated_by, created_date):
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO user_chats (user_id, text, generated_by, created_date)
            VALUES (%s, %s, %s, %s)
        """, (user_id, text, generated_by, created_date))
        conn.commit()
    except Exception as e:
...
def get_chat_history_for_user(user_id):
    """
    Fetches the last 20 chat interactions for a given user ID.

    Args:
        user_id (int): The ID of the user whose chat history is to be retrieved.

    Returns:
        list of dicts: A list of dictionaries, each representing a chat message.
    """
    conn = None
    try:
        conn = get_db_connection()
        ...
        cur.execute("""
            SELECT user_id, text, generated_by, created_date
            FROM user_chats
            WHERE user_id = %s
        ...
        chat_history = [{
            "userid": chat["user_id"],
            "generated_by": chat["generated_by"],
            "text": chat["text"],
            "created_time": chat["created_date"].strftime("%Y-%m-%d %H:%M:%S")
        } for chat in chats]
